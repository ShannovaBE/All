...
1. Run DB migrations (if needed)
   ...
   - Smoke-check migrations locally with: `DATABASE_URL=sqlite:///./migration_check.db alembic upgrade head`
2. Start the API
   - `uvicorn main:app --reload`
3. Create 2 users
...
Manual Checklist (Retention)

1. Confirm migrations are applied in production:
   - From `backend/`: `alembic upgrade head`
2. Configure the scheduled GitHub Action secrets:
   - `SHANNOVA_RETENTION_URL`
   - `SHANNOVA_RETENTION_ADMIN_USER_ID`
3. Run the retention workflow manually once from GitHub Actions.
4. Confirm expired datasets move to `restriction_status=retention_expired`.
