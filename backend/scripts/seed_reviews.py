...
from users.store import _load_all_users  # internal use for dev seeding
...
def main() -> None:
    ...
    users = list(_load_all_users().values())
    if not users:
        raise SystemExit("No users found in backend/users_store/users.json. Create users first.")
