...
def _build_record_from_kwargs(
    *,
    ...
    record: Dict[str, Any] = {
        ...
        "retention_expires_at": _format_optional_timestamp(_parse_optional_timestamp(retention_expires_at)),
    }
...
def _record_to_model_fields(record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "retention_policy": record.get("retention_policy") or extra.get("retention_policy") or {},
        "retention_expires_at": _parse_optional_timestamp(
            record.get("retention_expires_at") or extra.get("retention_expires_at")
        ),
    }
...
def _model_to_record(model: DatasetRecord) -> Dict[str, Any]:
    ...
    record = {
        ...
        "retention_policy": model.retention_policy or {},
        "retention_expires_at": _format_optional_timestamp(model.retention_expires_at),
    }
    ...
    *,
    ...
    with SessionLocal() as db:
        ...
        if retention_policy is not None:
            current_retention = dict(result.retention_policy or {})
            current_retention.update(retention_policy)
            result.retention_policy = current_retention
        if retention_expires_at is not None:
            result.retention_expires_at = _parse_optional_timestamp(retention_expires_at)
        db.commit()
        db.refresh(result)
        return _model_to_record(result)


def list_retention_due_records(now: Optional[datetime] = None, limit: Optional[int] = None) -> List[Dict[str, Any]]:
    cutoff = now or datetime.now(timezone.utc)
    with SessionLocal() as db:
        stmt = (
            select(DatasetRecord)
            .where(DatasetRecord.retention_expires_at.is_not(None))
            .where(DatasetRecord.retention_expires_at <= cutoff)
            .where(DatasetRecord.restriction_status == "active")
            .order_by(DatasetRecord.retention_expires_at.asc())
        )
        if limit:
