...
from retention import build_retention_policy, run_retention_sweep
...
async def upload_file(
    ...
    retention_policy, retention_expires_at = build_retention_policy()
    try:
        put_object(
            key=object_key,
            body=contents,
...
def run_admin_retention(
    ...
    requester = get_user_by_id(requester_id) if requester_id else None
    if not requester or not is_admin(user_id=requester.get("id"), username=requester.get("username")):
        _audit(
            request,
    ...
    result = run_retention_sweep(dry_run=payload.dry_run, limit=payload.limit)
    _audit(
        request,
        action="RUN_RETENTION_SWEEP",
        actor_id=requester["id"],
        ...
        purpose="retention_enforcement",
        result="SUCCESS",
        metadata={"dry_run": payload.dry_run, "due_count": result["due_count"]},
    )
    return result
